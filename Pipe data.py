...
flat_die = { 110: [3, 4, 5], 125: [3, 5, 6], 135: [3, 4, 5, 6, 7], 150: [5, 6, 7], 180: [5, 7, 10] };
cladding_die = { 27: [3], 34: [3, 4], 42: [4, 5, 6], 54: [4, 5, 6], 65: [5, 6, 7], 75: [5, 6, 7, 8], 90: [6, 7, 8, 10], 110: [6, 7, 8, 10] };
...
for pd in pipe_mold:
    for p, wall_thickness in flat_die.items():
        for s1 in wall_thickness:
            for ppd, pp_thickness in cladding_die.items():
                for s4 in pp_thickness:
                    # Equations for Pipe Stiffness
                    Y1 = s1 / 2.0;
                    Y2 = s1 + s4 + 0.9*ppd/2.0;
                    A1 = s1 * p;
                    A2 = (0.9*ppd/2.0 + s4)**2 * math.pi - (0.9*ppd/2.0)**2 * math.pi;
                    H = (Y1*A1 + Y2*A2)/(A1+A2);
                    Y11 = abs(H - Y1);
                    Y21 = abs(Y2 - H);
                    J1 = p*(s1)**3/12.0;
                    J2 = math.pi/64.0 * ((0.9*ppd + 2.0*s4)**4 - (0.9*ppd)**4);
                    J = (J1+A1*Y11**2 + J2+A2*Y21**2)/p;
                    Sn = 1000.0 * J * elastic_modulus / pd**3;
                    # print(Y1, Y2, A1, A2);
                    # print(H, Y11, Y21, J1);
                    # print(J2, J);
                    ...
                    V3 = ((pd/2.0+s1)**2-(pd/2.0)**2) * math.pi * pipe_length;
                    ...
                    if (math.trunc(Sn) in required_Sn):
                        ...
                        t_list = [pd, p, s1, ppd, s4, round(Sn, 2), round(W0, 2), pp_dist];
                        result.append(t_list);


print(result);

header = ['Pipe Diameter', 'Flat Die', 'Thickness', 'PP Diameter', 'PP Thickness', 'SN', 'Pipe Weight', 'PP Distance'];
