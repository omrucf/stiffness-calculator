...
class CTkScrollableFrame(
    ...
    def __init__(
        ...
        if self._orientation == "horizontal":
            ...
            self._parent_canvas.configure(xscrollcommand=self._scrollbarx.set)

            self._scrollbary = CTkScrollbar(
                master=self._parent_frame,
                orientation="vertical",
                command=self._parent_canvas.yview,
                fg_color=scrollbar_fg_color,
            ...
            self._parent_canvas.configure(yscrollcommand=self._scrollbary.set)
        ...
        self._label = CTkLabel(
            ...
            fg_color=ThemeManager.theme["CTkScrollableFrame"]["label_fg_color"]
            if label_fg_color is None
            else label_fg_color,
        )
        ...
        self.bind(
            "<Configure>",
            lambda e: self._parent_canvas.configure(
        ...
        if self._parent_frame.cget("fg_color") == "transparent":
            tkinter.Frame.configure(
                self,
                bg=self._apply_appearance_mode(self._parent_frame.cget("bg_color")),
            )
            self._parent_canvas.configure(
                bg=self._apply_appearance_mode(self._parent_frame.cget("bg_color"))
            )
            ...
            tkinter.Frame.configure(
                self,
                bg=self._apply_appearance_mode(self._parent_frame.cget("fg_color")),
            )
            self._parent_canvas.configure(
                bg=self._apply_appearance_mode(self._parent_frame.cget("fg_color"))
            )
    ...
    def _create_grid(self):
        ...
        if self._orientation == "horizontal":
            ...
            self._scrollbarx.grid(row=2, column=0, sticky="nsew", padx=border_spacing)
            ...
            self._scrollbary.grid(row=1, column=1, sticky="nsew", pady=border_spacing)
    ...
    def configure(self, **kwargs):
        ...
        if "scrollbar_button_hover_color" in kwargs:
            self._scrollbar.configure(
                button_hover_color=kwargs.pop("scrollbar_button_hover_color")
            )
    ...
    def _fit_frame_dimensions_to_canvas(self, event):
        if self._orientation == "horizontal":
            self._parent_canvas.itemconfigure(
                self._create_window_id, height=self._parent_canvas.winfo_height()
            )
        elif self._orientation == "vertical":
            self._parent_canvas.itemconfigure(
                self._create_window_id, width=self._parent_canvas.winfo_width()
            )
